...
export default function SalesPage() {
  ...
  const filtered = sales.filter((s) => {
    ...
    return matchesQuery && matchesPayment && isWithinDateRange(s.created_at);
  });

  const totalCash = filtered
    .filter((s) => s.payment_method === "cash")
    .reduce((sum, s) => sum + s.total, 0);
  const totalMpesa = filtered
  ...
  const creditOutstanding = filtered
    .filter((s) => s.payment_method === "credit")
  ...
  const columns: Column<Sale>[] = [
    ...
    { header: "Balance", render: (s) => formatCurrency(s.balance) },
    {
      ...
      render: (s) => (
        <span
          className={`text-[10px] font-medium px-1.5 py-0.5 rounded border capitalize ${PAYMENT_METHOD_COLORS[s.payment_method] ?? ""}`}
        >
    ...
    },
    {
      ...
      render: (s) => (
        <span
          className={`text-[10px] font-medium px-1.5 py-0.5 rounded border ${getStatusColor(s.status)}`}
        >
